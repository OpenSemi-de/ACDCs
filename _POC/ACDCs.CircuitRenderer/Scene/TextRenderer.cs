...
public sealed class TextRenderer : IRenderer<TextInstruction>, IRenderer
{
    public void Render(ICanvas canvas, RenderInstruction instruction, TextInstruction text) => s_Render(canvas, instruction, text);

    private static void s_Render(ICanvas canvas, RenderInstruction instruction, TextInstruction text)
    {
        Coordinate centerPos = new(text.Position);
        DrawableScene.SetStrokeColor(canvas, text.StrokeColor);
        canvas.FontColor = instruction.ForegroundColor != null
            ? instruction.ForegroundColor.ToMauiColor()
            : text.StrokeColor?.ToMauiColor();
